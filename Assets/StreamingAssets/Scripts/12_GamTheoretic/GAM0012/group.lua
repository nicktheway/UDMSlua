...
local ground 
local transDur = 0.2
local cm=UT.colMakeColorMap("parula")

local numStates = 3 --2,3,4,5
local gca
local threshold
...
function start()
	...
	threshold = 2 --1,2,3
	LFG.setState(0,threshold + 1)
	--gca=LFG.gcaDefine({0,2}, {0,2}, numStates, threshold)
	gca=LFG.gcaDefine({1,2}, {1,2}, numStates, threshold)
	onGcaStart()
end
...
function update()
	...
	for i = 0, Nagn-1 do
		...
		local col2=UT.stateToColor(LFG.getState(i),numStates+1,cm)
...
function onGcaStart()
	...
	for i=0,Nagn-1 do
		...
		s=LFG.getState(i)
		LFG.setColor(i,UT.stateToColor(s,numStates+1,cm))
	end
