...
function ThanStuff01()
	...
	for i=0, Nagn-1 do
		local agent=Members[i]
		local TR=agent.gameObject:AddComponent(typeof(UE.TrailRenderer))
		--agent:GetComponent(typeof(UE.TrailRenderer)).widthMultiplier=0.1
		TR.widthMultiplier=0.3
		TR.time=30
	end
	MyNuCam=UE.GameObject()
	MyNuCam.gameObject:AddComponent(typeof(UE.Camera))
...
function ThanUpdate()
	...
	MyNuCam.transform.position=UE.Vector3(4*(1+sin(0.05*TIME)),15,4*(1+cos(0.02*TIME)))
	MyNuCam.transform:LookAt(Members[0].transform.position+UE.Vector3(0,1,0))
	MyNuCam:SetActive(false)
end
function ThanMove(agentId)
	local m=math.fmod(agentId+1,Nagn)
	local x=transform.position.x
	local z=transform.position.z
	local R1=agentId*PosPar01(0.1*TIME,agentId); transform.position=R1
	--local R1=agentId*PosPol01(0.1*TIME,agentId); transform.position=R1
...
function individualPrepare(agentId) 
	...
	Members[agentId].TurnToMoveDir=agentId<Nagn/2
